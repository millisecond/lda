...
var lda = new function () {
    ...
    this.basicInference = function (doc) {
        var sums = makeArray(this.K);
        for (var n = 0; n < doc.length; n++) {
            var word = doc[n];
            for (var t = 0; t < this.K; t++) {
                sums[t] += this.phisum[t][word];
            }
        }

        var sum = 0;
        for (var t = 0; t < this.K; t++) {
            sums[t] = sums[t] / doc.length;
            sum += sums[t];
        }

        var topicAverages = makeArray(K);
        for (var t = 0; t < this.K; t++) {
            topicAverages[t] = sums[t] / sum;
        }
